...
import 'package:flutter_k_chart/utils/data_util_binance.dart';
...
class _MyHomePageState extends State<MyHomePage> {
  List<CandleChartEntity>? datasBinance;
    ...
    return Wrap(
      ...
      children: <Widget>[
        ...
        button('update', onPressed: () {
          ...
          // DataUtilBinance.updateLastData(datasBinance, maDayList);
        }),
        button('addData', onPressed: () {
          ...
          // datas!.last.high = max(datas!.last.high, datas!.last.close);
          // datas!.last.low = min(datas!.last.low, datas!.last.close);
          // DataUtilBinance.addLastData(datasBinance, kLineEntity, maDayList);
        }),
...
//              setState(() {});
          ...
          //     .toList()
          //     .cast<KLineEntity>();
          DataUtilBinance.calculate(datasBinance, maDayList);
        }),
  ...
  void getData({String? symbol, String? period}) async {
    ...
    try {
      ...
      final results = json.decode(result) as List<dynamic>;
      datasBinance = results
          .map((list) {
            return CandleChartEntity.fromApi(list);
          })
          .toList()
          .cast<CandleChartEntity>();
      // debugPrint('### $datasBinance');
      DataUtilBinance.calculate(datasBinance, maDayList);
      showLoading = false;
      setState(() {});
    }
  ...
  Future<String> getIPAddress({String? symbol, String? period}) async {
    ...
    final binance =
        'https://fapi.binance.com/fapi/v1/klines?symbol=${symbol ?? 'btcusdt'}&interval=${period ?? '1d'}';
    String result;
    final response =
